...
* @param {Integer} throttling_ms_int sets the minimum period (in milliseconds) between messages 
*
*/
...
{
  // Initialize the aux timestamp for the the throttling behavior
...
async function sendGoal(ngsi_command_value, ngsi_command_name){
  ...
  ros2_node.getLogger()
  .info(`Goal succeeded with result: ${JSON.stringify(result)}`);
  // Update the NGSI Command
  iotAgentLib.setCommandResult(device_id,
    iota_conf.default_resource,iota_conf.default_key,
    ngsiCommand.name,
      ...
      activeAttributes.map(function (interfaceDescriptor) {
        ...
        let subscription = ROS_Node.createSubscription(topicType, topicName, (msg) => {
            ...
            if (interval >= lastMessages[subscriberName].throttling) {
                ...
                var sendToBroker = true;

                try {
                  prevAttrValue = incomingRosDevice.attribute.value;
                  sendToBroker = JSON.stringify(prevAttrValue) != JSON.stringify(attribute.value);
                } catch (notFoundError) {
                }

                incomingRosDevice.attribute = attribute;

                if (sendToBroker) {
                  iotAgentLib.update(incomingRosDevice.device.name, incomingRosDevice.device.type, 
                                      '', [attribute], incomingRosDevice.device, function (error) {
                      if (error) {
